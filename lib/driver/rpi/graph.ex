...
# just as well been in Scenic.Driver.Rpi itself, but that was getting too long
# and complicated
#
defmodule Scenic.Driver.Rpi.Graph do
...
#    send_input_flags( input_flags, port )
  ...
  defp render_graphs( keys, %{
    ...
    Task.start_link fn ->
      ...
      if root_id, do: Rpi.Port.set_root_dl(port, root_id)
    end
  ...
  defp ensure_one_id( graph_key, state ) do
    case get_dl_id( graph_key, state ) do
      nil ->
        ...
        # {:ok, dl_id} = Rpi.Port.new_dl_id(port)
